...
//  ToDoList
//
//  Created by Teddy Weaver on 9/26/21.
//
...
class ToDoDetailTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        if toDoItem == nil{
            toDoItem = ToDoItem(name: nameField.text!, date: datePicker.date, notes: noteView.text, reminderSet: reminderSwitch.isOn, completed: false)
        ...
        LocalNotificationManager.isAuthorized { (authorized) in
            DispatchQueue.main.async {
                if !authorized && self.reminderSwitch.isOn {
                    self.oneButtonAlert(title: "User Has Not Allowed Notifications", message: "Go to Settings app.")
